...
public class MapperUtil {
	...
	// private static volatile MapperUtil mapperUtil = null;
	...
	private static class SingletonHolder {
		private static final MapperUtil INSTANCE = new MapperUtil();
	}
	public static MapperUtil getInstance() {
	...
	private String createResultMap(String entitySpace, boolean anyJDBC, SuperAttribute attr) {
		...
		result.append("    <resultMap id=\"result_"
			+ attr.getClassName()
			+ "_Map\" type=\""
			+ entitySpace
			+ "."
	...
	private StringBuffer addSonMap(String entitySpace, List<ColumnItem> item, boolean anyJDBC) {
		...
		for (ColumnItem col : item) {
			...
			if (list != null) {
				...
				result.append("        <association property=\""
					+ col.getInPropertyName()
					+ "\" javaType=\""
					+ entitySpace
					+ "."
					+ col.getClassName()
		...
		for (ColumnItem col : item) {
			...
			if (list != null) {
				...
				result.append("        <collection property=\""
					+ col.getInPropertyName()
					+ "\" ofType=\""
					+ entitySpace
					+ "."
				...
				for (int i = 0; i < size; i++) {
					if (StringUtils.isEmpty(list.get(i).getConlumn()) || list.get(i).getCheck() == false) {
						continue;
					}
					if (i == 0 && list.get(i).getConlumn().equals(col.getPrimaryKey())) {
						if (col.getTableAlias() != null && !(col.getTableAlias().isEmpty())) {
	...
	private String createSql(boolean anyAssist, SuperAttribute attr) {
		...
		int size = item.size();
		for (int i = 0; i < size; i++) {
			if (StringUtils.isEmpty(item.get(i).getConlumn()) || item.get(i).getCheck() == false) {
				continue;
			}
			if (attr.getColumnItems() != null) {
				if (i == 0) {
		...
		for (ColumnItem item : items) {
			...
			if (cvf != null) {
				...
				for (int i = 0; i < cvf.size(); i++) {
					if (StringUtils.isEmpty(cvf.get(i).getConlumn())) {
						continue;
					}
					if (i == 0) {
	...
	private String getJoinStr(List<ColumnItem> attrs, String original, int spaceCount) {
		...
		for (ColumnItem item : attrs) {
			if (item.getJoinTableName() == null
				|| "".equals(item.getJoinTableName())
				|| item.getJoinColumn() == null
			...
			result.append(item.getJoinType()
				+ " join "
				+ item.getTableName()
				+ " on "
				+ item.getTableName()
				...
				+ item.getJoinColumn()
				+ "="
				+ item.getJoinTableName()
				+ "."
				+ item.getJoinColumn()
				+ "\r\n");
			if (item.getGrandItem() != null) {
				for (ColumnItem itemg : item.getGrandItem()) {
					if (itemg.getJoinTableName() == null
						|| "".equals(itemg.getJoinTableName())
						|| itemg.getJoinColumn() == null
						|| "".equals(itemg.getJoinColumn())
					...
					result.append(itemg.getJoinType()
						+ " join "
						+ itemg.getTableName()
						+ " on "
						+ itemg.getTableName()
	...
	private String createInsertNonEmpty(SuperAttribute attr, String entitySpace, boolean anyJDBC) {
		...
		result.append("    <insert id=\"insertNonEmpty"
			+ attr.getClassName()
			+ "\" parameterType=\""
			+ entitySpace
			+ "."
	...
	private String createDeleteById(SuperAttribute attr, String entitySpace, boolean anyJDBC) {
		...
		if (attr.getPrimaryKey() != null) {
			...
			if (javaIdType != null) {
				...
				result.append("    <delete id=\"delete"
					+ attr.getClassName()
					+ "ById\" parameterType=\"java.lang."
					+ javaIdType
					+ "\">\r\n");
	...
	private String createDeleteByAssist(SuperAttribute attr, String assistSpace) {
		...
		result.append(
			"    <delete id=\"delete" + attr.getClassName() + "\" parameterType=\"" + assistSpace + ".Assist\">\r\n");
		result.append("        delete from " + attr.getTableName() + "\r\n");
		result.append("        <if test=\"require!=null\"><include refid=\"Assist\" /></if>\r\n");
		result.append("    </delete>\r\n\r\n");
		return result.toString();
	...
	private String createUpdateById(SuperAttribute attr, String entitySpace, boolean anyJDBC) {
		...
		if (attr.getPrimaryKey() != null) {
			...
			result.append("    <update id=\"update"
				+ attr.getClassName()
				+ "ById\" parameterType=\""
				+ entitySpace
				+ "."
			...
			for (int i = 0; i < cvf.size(); i++) {
				if (cvf.get(i).getConlumn() != null) {
					if (attr.getPrimaryKey().equals(cvf.get(i).getConlumn())) {
						...
						if (falg) {
							result.append(
								"            " + cvf.get(i).getConlumn() + "=" + "#{" + cvf.get(i).getPropertyName());
							if (anyJDBC) {
								if (cvf.get(i).getJdbcType() != null) {
									result.append(",jdbcType=" + cvf.get(i).getJdbcType());
								}
	...
	private String createUpdateNonEmptyById(SuperAttribute attr, String entitySpace, boolean anyJDBC) {
		...
		if (attr.getPrimaryKey() != null) {
			result.append(
				"    <!-- 通过" + attr.getClassName() + "的id将" + attr.getClassName() + "中属性值不为null的数据更新到数据库对应的表中-->\r\n");
			result.append("    <update id=\"updateNonEmpty"
				+ attr.getClassName()
				+ "ById\" parameterType=\""
				+ entitySpace
				+ "."
	...
	private String createUpdateByAssist(SuperAttribute attr, boolean anyJDBC) {
		...
		if (attr.getPrimaryKey() != null) {
			...
			for (int i = 0; i < cvf.size(); i++) {
				if (cvf.get(i).getConlumn() != null) {
					result.append(
						"                " + cvf.get(i).getConlumn() + "=" + "#{enti." + cvf.get(i).getPropertyName());
					if (anyJDBC) {
						if (cvf.get(i).getJdbcType() != null) {
							result.append(",jdbcType=" + cvf.get(i).getJdbcType());
						}
